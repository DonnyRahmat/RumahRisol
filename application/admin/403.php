...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <p>> <span>SOME PAGES ON THIS SERVER THAT YOU DO HAVE PERMISSION TO ACCESS</span>:
      ...
      <?php
        $sql = "SELECT U.fullname, R.nama_modul, R.link_modul from t_rolemod R left join t_detil_role DR on DR.id_rolemod=R.id_rolemod left join t_role RO on RO.id_role=DR.id_role left join t_user U on RO.id_role=U.role where U.role=:idrole group by DR.id_detil_role";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':idrole', $_SESSION['idrole']);
        $stmt->execute();
        $hasil = $stmt->fetchAll();
        foreach ($hasil as $h) {
          echo "<a href=".base."".$h['link_modul'].">".$h['nama_modul']."</a>, ";
        }
      ?>
      ]
    </p><p>> <span>HAVE A NICE DAY :-)</span></p>
    </div>
  </body>
